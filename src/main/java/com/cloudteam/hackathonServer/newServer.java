	...
	static class loginHandler extends myHandler {
		...
		public void handle(HttpExchange t) throws IOException {
			...
			case SuccessCode_LOGIN:
				t.sendResponseHeaders(SuccessCode_LOGIN,
			...
			case ErrorCode_EMPTY_REQUEST:
				...
				t.sendResponseHeaders(ErrorCode_EMPTY_REQUEST - 10,
						response.getBytes().length);
				break;
			default:
				break;
	...
	static class cartsHandler extends myHandler {
		...
		public void handle(HttpExchange t) throws IOException {
			...
			if (check.checkToken()) {
				...
				response = carts.CreateCartsHand();

				res = JSONObject.fromObject(response);
				// //获取请求数据，非阻塞
				// BufferedReader body = new BufferedReader(new
				// InputStreamReader(t.getRequestBody()));
				// String data = body.readLine();

				// 获取请求方法 POST/GET
				System.out.println(t.getRequestMethod() + " " + res + "\n");
			} else {
				// reponse 401
			}
	...
	static class foodsHandler extends myHandler {
		...
		public void handle(HttpExchange t) throws IOException {
			...
			if (token == null) {
				...
				t.sendResponseHeaders(ErrorCode_INVALID_ACCESS_TOKEN, response.getBytes().length);
			} else {
				//token不为空
				JSONObject res = new JSONObject();
				TokenCheck check = new TokenCheck(token);
